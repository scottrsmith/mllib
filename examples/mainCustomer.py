...
project.importFile('transactions', type='csv', description='Clustering model', fileName='int_online_tx.csv',  hasHeaders = True)
...
print project.explore['transactions']
print project.explore['transactions'].allStatsSummary()
project.explore['transactions'].plotExploreHeatMap()

#project.explore.plotHistogramsAll(10)
project.explore['transactions'].plotCategoryColumn('Country')


print project.dataFile['transactions'].dataFrame.shape
...
project.addManualRuleForDefault(ed.EXPLORE_PCA_DIMENSIONALITY_REDUCTION,['StockCode','CustomerID'], 'pca_item_data')

project.addManualRuleForDefault(ed.EXPLORE_THRESHOLD_DIMENSIONALITY_REDUCTION,['StockCode','CustomerID'], 'threshold_item_data')
